...
def create_convex_polygn_price_process(
    ...
    # linear
    samples = [
        minimum_polygn_price + (10 - minimum_polygn_price) * i / (timesteps * dt + 2)
        for i in range((timesteps * dt + 2))
    ]
    # qudratic convex
    t = timesteps * dt
    ...
    target_avg = 5
    samples = [
        (sample - minimum_polygn_price)
        / curr_average
        * (target_avg - minimum_polygn_price)
        + minimum_polygn_price
...
def create_moore_s_law_hardware_cost(
    ...
    hardware_cost = [
        init_hardware_cost / (2 ** (i / t * final_year / moore_low_year_half_period))
        for i in range(t)
    ]
...
def create_stochastic_process_realizations(
    ...
    switcher = {
        "convex_polygn_price_samples": [
            create_convex_polygn_price_process(
                timesteps=timesteps, dt=dt, rng=rng_generator()
            )
            for _ in range(runs)
        ],
        "stochastic_polygn_price_samples": [
            create_stochastic_polygn_price_process(
                timesteps=timesteps, dt=dt, rng=rng_generator()
            )
            for _ in range(runs)
        ],
        "adoption_rates": [
            create_exp_adoption_rate(
                timesteps=timesteps,
                dt=dt,
                rng=rng_generator(),
                final_chains_num=final_chains_num,
            )
            for _ in range(runs)
        ],
        "validator_samples": [
        ...
        "hardware_costs": [
            create_moore_s_law_hardware_cost(
                timesteps=timesteps,
                dt=dt,
                rng=rng_generator(),
                init_hardware_cost=init_hardware_cost,
            )
            for _ in range(runs)
        ],
    }
...
def create_intial_state_risk_service_validator(
    public_chain_cnt, private_chain_cnt, validator_cnt
):
    ...
    for i in range(chain_cnt):
        ...
        zero_indices = np.where(matrix_restaking[i] != 0)[
            0
        ]  # indices where the row is zero
        if len(zero_indices) < non_zero_cnt:
            ...
            non_zero_indices = np.random.choice(
                list(range(len(matrix_restaking[i]))), non_zero_cnt, replace=False
            )  # random select zero indices
